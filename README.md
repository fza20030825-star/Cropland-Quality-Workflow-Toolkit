...
Cropland Quality Workflow Toolkit 是一个面向 ArcGIS Pro Python 环境的耕地质量评价流程工具集。项目把高标准农田隶属度计算、现有农田面更新、三调耕地图斑更新和面积平差串联为 4 个正式流程，并提供统一 Tkinter 图形界面。工具集还提供辅助工具，用于国标字段转三普法隶属度、从三普单元图补耕层厚度，以及在人工修改第三步结果原始指标后重算隶属度。
...
统一界面顶部可切换 4 个正式流程和 3 个辅助工具。顶部“统一输出 GDB”选择一次后，会同步到各步骤和辅助工具的输出 GDB；在任意步骤内部改输出 GDB，也会同步回全部页面。任意页面选择国标二级农业区后，也会同步到其他带农业区选择的工具。各页面只需要点击“提交...”按钮：工具会自动完成提交前审查，审查通过后弹出确认报告，用户确认后才真正提交任务；审查失败会显示报告并中断，不再叠加额外错误弹窗。第一步勾选或取消勾选高标面数据后，投影确认区会立即刷新所选数据的投影和统一投影来源。正式流程前一步成功输出后，会自动把结果路径填入下一步对应输入框并切换页面；辅助工具成功后只停留在当前辅助页面，不自动跳转。其他业务输入仍由用户选择或填写。底部“详细信息”区域集中显示审查提示、任务状态、日志路径和自动衔接信息。“恢复默认输入”只恢复各步骤输入与参数，不清空详细信息。数据列表、投影检查和详细信息等区域都有局部滚动条，便于查看长路径和长报告。
...
辅助工具不改变 4 个正式流程的业务顺序，主要用于处理历史数据、国标法高标项目数据或人工修订后的第三步结果。
...
| 重算隶属度 | 输入人工修改过原始指标的第三步结果，检查固定 39 字段结构，并按所选国标二级农业区重新计算 13 个 `F...`、`评价得分` 和 `质量等级`。 |
...
使用标记模式时，任务完成后会在详细信息中提示 `是否重叠` 字段是否全为 `0`；如果存在重叠，会显示 `是否重叠=1` 的要素数量。
...
第一步会按用户选择的国标二级农业区读取规则表。若选择区域不是 `秦岭大巴山林农区` 或 `渝鄂湘黔边境山地林农牧区`，允许输入数据缺少 `海拔高度` 字段，已有 `海拔高度` 字段也允许为空；该口径会延续到第二步、第三步审计。其他 12 个指标必须完整匹配并有值。
